...
const DEPT_TREE: DeptNode[] = [
  {
    ...
    headEmail: "rajesh.kumar@company.com",
    headDesignation: "VP Engineering",
    description:
      "Responsible for building and maintaining all software products, infrastructure, and technical systems across the organization.",
    locations: ["Bangalore", "Mumbai", "Remote"],
    costCenter: "ENGR-001",
    parentId: null,
    children: [
      {
        id: "frontend", name: "Frontend", headcount: 89, dotColor: "bg-blue-400",
        openPositions: 3, headName: "Arun Kumar", headEmail: "arun.kumar@company.com",
        headDesignation: "Frontend Lead", description: "Builds user-facing web interfaces.", locations: ["Bangalore", "Remote"],
        costCenter: "ENGR-FE", parentId: "engineering",
      },
      {
        id: "backend", name: "Backend", headcount: 120, dotColor: "bg-indigo-500",
        openPositions: 5, headName: "Karan Mehta", headEmail: "karan.mehta@company.com",
        headDesignation: "Backend Lead", description: "Designs and maintains APIs and data systems.", locations: ["Bangalore", "Mumbai"],
        costCenter: "ENGR-BE", parentId: "engineering",
      },
      {
        id: "devops", name: "DevOps", headcount: 45, dotColor: "bg-violet-500",
        openPositions: 2, headName: "Vikram Singh", headEmail: "vikram.singh@company.com",
        headDesignation: "DevOps Lead", description: "Manages infrastructure, CI/CD, and cloud operations.", locations: ["Pune", "Remote"],
        costCenter: "ENGR-DO", parentId: "engineering",
      },
      {
        id: "qa", name: "QA", headcount: 86, dotColor: "bg-cyan-500",
        openPositions: 4, headName: "Priya Nair", headEmail: "priya.nair@company.com",
        headDesignation: "QA Lead", description: "Ensures quality across all product releases.", locations: ["Bangalore", "Hyderabad"],
        costCenter: "ENGR-QA", parentId: "engineering",
      },
    ],
  },
...
const BUDGET_MONTHS: BudgetMonth[] = [
  { month: "Jan 2026", budget: 40, actual: 38.2, variance: 1.8, status: "Under" },
  { month: "Feb 2026", budget: 40, actual: 41.2, variance: -1.2, status: "Over" },
  { month: "Mar 2026", budget: 42, actual: 39.5, variance: 2.5, status: "Under" },
  { month: "Apr 2026", budget: 42, actual: 43.1, variance: -1.1, status: "Over" },
  { month: "May 2026", budget: 45, actual: 44.2, variance: 0.8, status: "Under" },
  { month: "Jun 2026", budget: 45, actual: 42.8, variance: 2.2, status: "Under" },
];

const EMPLOYMENT_PIE = [
  { name: "Full Time", value: 280, color: "#3b82f6" },
  { name: "Contract", value: 35, color: "#8b5cf6" },
  { name: "Intern", value: 25, color: "#f97316" },
];
...
function OverviewTab({ dept }: { dept: DeptNode }) {
  ...
  const totalHeadcount = dept.headcount;
  const avgTenure = "3.2 yrs";
  const attrition = "8.4%";
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          { icon: Users, label: "Total Headcount", value: totalHeadcount.toString(), bg: "bg-blue-500" },
          { icon: Briefcase, label: "Open Positions", value: dept.openPositions.toString(), bg: "bg-emerald-500" },
          { icon: Clock, label: "Avg Tenure", value: avgTenure, bg: "bg-violet-500" },
          { icon: TrendingDown, label: "Attrition (12mo)", value: attrition, bg: "bg-rose-500" },
        ...
        <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
          ...
          {!editing ? (
            <div className="space-y-4 p-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <a href={`mailto:${dept.headEmail}`} className="text-xs text-blue-600 hover:underline">
                    {dept.headEmail}
                  </a>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-700">
                    {dept.parentId ? (findDept(dept.parentId)?.name ?? "—") : <span className="text-slate-400">None (Top-level)</span>}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3 p-5">
              {[
                { label: "Head of Dept", field: "headName" as const },
                { label: "Head Email", field: "headEmail" as const },
                { label: "Head Designation", field: "headDesignation" as const },
                { label: "Cost Center", field: "costCenter" as const },
                { label: "Locations (comma-separated)", field: "locations" as const },
              ].map(({ label, field }) => (
                <div key={field}>
              ...
              <tbody className="divide-y divide-slate-100">
                {dept.children.map((sub) => (
                  <tr key={sub.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-5 py-3">
                      <span className="font-semibold text-slate-900">{sub.headcount}</span>
                    </td>
                    <td className="px-5 py-3">
...
function MemberCard({
  ...
  return (
    <div
      onClick={(e) => onCardClick(member.id, e)}
      ...
      <div className="mb-2 flex items-start justify-between">
        <div className="relative">
          ...
          <span className="absolute -bottom-0.5 -right-0.5">
            <OnlineDot status={member.onlineStatus} />
          </span>
        </div>
...
function MemberPopover({ member, onClose }: { member: DeptMember; onClose: () => void }) {
  ...
  return (
    <div
      ref={ref}
      className="absolute z-50 w-64 rounded-xl border border-slate-200 bg-white p-4 shadow-xl"
          ...
          {sortOpen && (
            <div className="absolute right-0 top-full z-50 mt-1 w-36 rounded-lg border border-slate-200 bg-white py-1 shadow-lg">
              {["Name A-Z", "Name Z-A", "Join Date"].map((opt) => (
                <button key={opt} onClick={() => { setSortBy(opt); setSortOpen(false); }} className={`block w-full px-3 py-1.5 text-left text-xs hover:bg-slate-50 ${sortBy === opt ? "font-semibold text-blue-600" : "text-slate-700"}`}>
                  {opt}
                </button>
              ))}
            </div>
      ...
      <div className="grid grid-cols-5 gap-3">
        {filtered.map((member) => (
          <div key={member.id} className="relative">
            <MemberCard
              member={member}
              onCardClick={(id) => setPopoverId((prev) => (prev === id ? null : id))}
            />
            {popoverId === member.id && (
              <MemberPopover member={member} onClose={() => setPopoverId(null)} />
            )}
          </div>
        ))}
      </div>
...
function BudgetTab({ onRequestChange }: { onRequestChange: () => void }) {
  const totalBudget = BUDGET_MONTHS.reduce((s, m) => s + m.budget, 0);
  const totalActual = BUDGET_MONTHS.reduce((s, m) => s + m.actual, 0);
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <p className="text-sm font-bold text-slate-800">FY 2026 Budget Overview</p>
          ...
          >
            <DollarSign className="h-3.5 w-3.5" /> Request Budget Change
          </button>
        </div>
        <div className="mb-3 flex items-end justify-between text-xs">
          <span className="text-slate-500">Actual Spend: <span className="font-bold text-slate-900">{formatLakh(totalActual)}L</span></span>
          <span className="text-slate-500">Budget: <span className="font-bold text-slate-900">{formatLakh(totalBudget)}L</span></span>
        </div>
        <div className="h-3 overflow-hidden rounded-full bg-slate-100">
          <div
            className={`h-full rounded-full transition-all ${pct > 100 ? "bg-red-500" : pct > 90 ? "bg-amber-500" : "bg-blue-500"}`}
            style={{ width: `${Math.min(pct, 100)}%` }}
          />
        </div>
        <p className="mt-1.5 text-right text-xs text-slate-500">{pct}% utilized</p>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50 text-slate-500">
                {["Month", "Budget (L)", "Actual (L)", "Variance (L)", "Status"].map((h) => (
                  <th key={h} className="px-5 py-3 font-semibold">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {BUDGET_MONTHS.map((row) => (
                <tr key={row.month} className="hover:bg-slate-50">
                  <td className="px-5 py-3 font-medium text-slate-700">{row.month}</td>
                  <td className="px-5 py-3 text-slate-700">₹{row.budget}L</td>
                  <td className="px-5 py-3 font-semibold text-slate-900">₹{row.actual}L</td>
                  <td className={`px-5 py-3 font-semibold ${row.variance >= 0 ? "text-green-600" : "text-red-600"}`}>
                    {row.variance >= 0 ? "+" : ""}₹{row.variance}L
                  </td>
                  <td className="px-5 py-3">
                    <span className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${row.status === "Under" ? "bg-green-50 text-green-700" : "bg-red-50 text-red-600"}`}>
                      {row.status}
                    </span>
                  </td>
                </tr>
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-100 px-5 py-3">
          <p className="text-sm font-bold text-slate-800">Headcount Cost by Employment Type</p>
        </div>
...
function AddDepartmentModal({ onClose }: { onClose: () => void }) {
  ...
  function inputCls(field: string) {
    return `h-9 w-full rounded-lg border px-3 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-100 ${errors[field] ? "border-red-400" : "border-slate-200 focus:border-blue-400"}`;
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl">
        ...
        <div className="space-y-4 p-6">
          ...
          <div>
            <label className="mb-1 block text-xs font-semibold text-slate-700">Description</label>
            <textarea rows={2} value={form.description} onChange={(e) => setField("description", e.target.value)} placeholder="Brief description..." className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-700 focus:border-blue-400 focus:outline-none" />
          </div>
          <div>
...
function BudgetChangeModal({ onClose }: { onClose: () => void }) {
  const [form, setForm] = useState({ type: "Increase", amount: "", reason: "", period: "" });

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
          <p className="text-base font-bold text-slate-900">Request Budget Change</p>
          <button onClick={onClose} className="grid h-7 w-7 place-items-center rounded-lg text-slate-400 hover:bg-slate-100"><X className="h-4 w-4" /></button>
        </div>
        <div className="space-y-4 p-6">
          <div>
            <label className="mb-1 block text-xs font-semibold text-slate-700">Request Type</label>
            <div className="flex gap-2">
              {["Increase", "Decrease", "Reallocation"].map((t) => (
                <button
                  key={t}
                  onClick={() => setForm((f) => ({ ...f, type: t }))}
                  className={`rounded-lg border px-3 py-1.5 text-xs font-semibold transition-colors ${form.type === t ? "border-blue-500 bg-blue-50 text-blue-700" : "border-slate-200 text-slate-600 hover:bg-slate-50"}`}
                >
                  {t}
                </button>
              ))}
          ...
          <div>
            <label className="mb-1 block text-xs font-semibold text-slate-700">Amount (in Lakhs)<span className="ml-0.5 text-red-500">*</span></label>
            <input
              type="number"
              value={form.amount}
          ...
          <div>
            <label className="mb-1 block text-xs font-semibold text-slate-700">Period</label>
            <input
              type="month"
              value={form.period}
...
type DeptView = "structure" | "headcount";

interface DepartmentsProps {
  defaultView?: DeptView;
}

export default function Departments({ defaultView }: DepartmentsProps) {
  const initialTab = defaultView === "headcount" ? "Budget" : "Overview";
  ...
  const [activeTab, setActiveTab] = useState<"Overview" | "Members" | "Budget">(initialTab as "Overview" | "Budget");
  ...
  return (
    <div className="flex min-h-full bg-slate-50">
      ...
      <main className="flex-1 min-w-0 px-6 py-5">
        ...
        <div className="mb-5">
          <nav className="mb-1.5 flex items-center gap-1.5 text-xs text-slate-500">
            ...
            <span>›</span>
            <span className="font-semibold text-slate-700">{selectedDept.name}</span>
          </nav>

          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <span className={`h-4 w-4 rounded-full ${selectedDept.dotColor}`} />
        ...
        <div className="mb-5 border-b border-slate-200">
          <div className="flex items-center gap-1">
            {(["Overview", "Members", "Budget"] as const).map((tab) => (
              <button
                key={tab}
        ...
        {/* Tab content */}
        {activeTab === "Overview" && <OverviewTab dept={selectedDept} />}
        {activeTab === "Members" && (
          <MembersTab
            members={ENG_MEMBERS}
        ...
        {activeTab === "Budget" && (
          <BudgetTab onRequestChange={() => setShowBudgetModal(true)} />
        )}
      </main>
      ...
      {showBudgetModal && <BudgetChangeModal onClose={() => setShowBudgetModal(false)} />}
    </div>
  );
}
